...
using System.Text;
using Tony.Common.Message;
using Tony.Common.Properties;

namespace Tony.Common.Composition
{
    /// <summary>
    /// 
    /// </summary>
    public static class ComposerFactory
    {
        ...
        public static void Init()
        {
            var action = new Action<FileInfo>(f =>{
                ...
                SystemMarkerAttribute attr = assembly.GetCustomAttributes(typeof(SystemMarkerAttribute), false)
                    .Cast<SystemMarkerAttribute>()
                    .FirstOrDefault();
                if(attr == null)
                {
                    throw new Exception();
        ...
        private static Assembly GetAssembly(string systemId)
        {
            ...
            {
                var assembly = Assembly.LoadFile(f.FullName);
                var attr = assembly.GetCustomAttributes(typeof(SystemMarkerAttribute), true).Cast<SystemMarkerAttribute>().FirstOrDefault();
